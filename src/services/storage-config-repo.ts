...
export async function isRegistered(db: ESADatabase): Promise<boolean> {
  const row = await db.prepare('SELECT value FROM config WHERE key = ?').bind('registered').first<{ value: string }>();
  return row?.value === 'true';
}

export async function getConfigValue(db: ESADatabase, key: string): Promise<string | null> {
  const row = await db.prepare('SELECT value FROM config WHERE key = ?').bind(key).first<{ value: string }>();
  return typeof row?.value === 'string' ? row.value : null;
}

export async function setConfigValue(db: ESADatabase, key: string, value: string): Promise<void> {
  await db
...
export async function setRegistered(db: ESADatabase): Promise<void> {
